...
export default function ItemsPage() {
  ...
  const categories = useMemo(() => [...new Set(items.map((i) => i.category_pt))].sort(), [])
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <p className="text-sm text-[var(--muted)] mb-4">
        {translatedCount}/{items.length} traduzidos ({((translatedCount / items.length) * 100).toFixed(1)}%)
      </p>

      <div className="flex flex-wrap gap-3 mb-6">
        <Input placeholder="Buscar item..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-56" />
        ...
        {filtered.map((i) => (
          <div key={i.key} className={`rounded-xl border border-[var(--border)] bg-[var(--card)] p-4 shadow-sm hover:shadow-lg hover:border-[var(--accent)]/50 transition-all duration-200 ${!i.translated ? 'opacity-70' : ''}`}>
            <div className="flex justify-between items-start mb-2">
              ...
              {i.cost && <span className="text-xs font-bold text-[var(--accent)]">{i.cost} PO</span>}
            </div>
            <div className="grid grid-cols-2 gap-1 text-[11px] text-[var(--muted)] mb-2 border-b border-[var(--border)] pb-2">
              ...
              <div><strong>Peso:</strong> {i.weight ? `${i.weight} lb` : '—'}</div>
              {i.weapon && <div><strong>Dano:</strong> {i.weapon.damage_dice} {i.weapon.damage_type_pt}</div>}
              {i.weapon && <div><strong>Propr.:</strong> {i.weapon.properties.join(', ') || '—'}</div>}
